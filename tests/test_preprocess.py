...
def test_load_config(mock_environment):  # pylint: disable=redefined-outer-name
...
def test_get_local_filepaths(mock_environment):  # pylint: disable=redefined-outer-name
    """Tests that only files on or after the 'date_since' are found."""
    preprocessor = JsonPreprocessor(config_path=mock_environment)
    filepaths = preprocessor.get_local_filepaths()
    expected_length = 2

    assert len(filepaths) == expected_length
...
def test_record_count(mock_environment):  # pylint: disable=redefined-outer-name
    """Tests that the number of records in a single file is counted correctly."""
    preprocessor = JsonPreprocessor(config_path=mock_environment)
    # Get the path to one of our mock files
    json_path = os.path.join(
        preprocessor.config["paths"]["local_json_dir"], "20250615_output.json"
    )
    expected_length = 2
...
def test_flatten_llm_json_to_dataframe(
    mock_environment,
...
def test_count_all_records(mock_environment):  # pylint: disable=redefined-outer-name
    ...
    expected_length = 3
    assert total_count == expected_length


def test_process_files(mock_environment):  # pylint: disable=redefined-outer-name
    ...
    assert (
        combined_df[combined_df["unique_id"] == "id_01"]["chosen_sic_code"].iloc[0]
        == "1111"
    )


def test_merge_eval_data(mock_environment):  # pylint: disable=redefined-outer-name
    ...
    # The merge is an "inner" join, so it should only contain ids present in both files
    # (id_01, id_03)
    expected_length = 2
    assert len(merged_df) == expected_length
