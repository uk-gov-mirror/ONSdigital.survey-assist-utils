...
class LabelAccuracy:
    ...
    def get_jaccard_similarity(self, match_type: str = "full") -> float:
        """Compute the average Jaccard Similarity Index between model-generated and clerical labels across all rows in the DataFrame.

        The Jaccard Similarity Index is calculated for each row by comparing sets of codes from model and clerical labels.
        Codes are optionally truncated to a specified length before comparison, depending on the `match_type`.
        ...
        Returns:
            float: The mean Jaccard Similarity Index across all rows. If both sets are empty for a row, the similarity is considered 1.0.
        """
        ...
        def calculate_jaccard_for_row(row):
            ...
            Args:
                row (pd.Series): A row from the DataFrame containing model and clerical label columns.
