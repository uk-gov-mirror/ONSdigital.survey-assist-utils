...
loading and flattening raw JSON output files generated by the Survey Assist
model into structured pandas DataFrames.

The processor extracts key fields such as unique identifiers, classification
status, selected SIC codes, and a configurable number of candidate SIC codes
with associated likelihood scores. This flattened structure enables easier
merging with ground truth data and downstream evaluation.

Typical usage:
    processor = JsonProcessor()
    df = processor.flatten_json_file("path/to/output.json", max_candidates=5)
"""
