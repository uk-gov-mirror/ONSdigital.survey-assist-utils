...
# Note: Due to a current bug in the codebase, we cannot currently calculate 'O'-type SA accuracy metrics.
#       The only currently available ones are OM or MM (one-CC & many-SA or many-CC & many-SA).
...
my_dataframe["alt_sic_candidate_parsed_extended"] = my_dataframe["alt_sic_candidate_parsed"]
